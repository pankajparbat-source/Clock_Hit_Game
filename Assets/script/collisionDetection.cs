...
public class collisionDetection : MonoBehaviour
{
        ...
        if (collision.gameObject.CompareTag("clock"))
        {
          active = GameObject.FindGameObjectsWithTag("active");
            if(active.Length>0)
            {
              ...
              gameManeger.Instance.removeFromList(active[0]);
            ...
            if (clock != null)
            {
                ...
                for(int index=1;index<=id;index++)
                {
                    if(index==1)
                    {
                        //clock.GetComponent<SpriteRenderer>().sprite = activeFaceImage;
                        //Task.Delay(8000);
                        clock.GetComponent<SpriteRenderer>().sprite = activeImage;
                        clock.tag = "active";
                        GameObject childclock = clock.transform.GetChild(0).gameObject;
                        clockHand = childclock.transform.GetChild(0).gameObject;
    ...
    {
        Destroy(gameObject);
        UIManeger.Instance.mainMenuScreanLoseCanvas();
    }
